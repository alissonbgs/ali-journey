...
The Journey page reads role data from `src/features/journey/data.ts`. Several non-current roles have placeholder-like summaries and outdated highlights that do not reflect actual responsibilities. The requested update is content-only: improve `summary` and `highlights` text while preserving timeline, company names, stack summaries, and technology lists.

## Goals / Non-Goals

**Goals:**
- Update summaries and highlights for Find My Profession, Smarkio, Embraer, and Black Bee Drones to be direct and meaningful.
- Keep all non-summary/non-highlights fields unchanged for those entries.
...
- No edits to role dates, company names, stack summary, or technologies.
- No new dependencies, data model changes, or API changes.

## Decisions

- **Edit only `summary` and `highlights` fields in one source file (`src/features/journey/data.ts`).**
...
- **Derive summaries and highlights strictly from user-provided role notes.**
  - Rationale: Ensures the final text matches real responsibilities and outcomes.
  - Alternative considered: rewriting content with inferred details; rejected because user-provided details are source-of-truth.
...
- **[Risk]** Accidental edits to protected fields (dates/company/stack/tech).  
  **Mitigation:** Limit patch scope to `summary` and `highlights` lines and review diff before finalizing.

## Migration Plan

1. Update summaries and highlights in `src/features/journey/data.ts`.
2. Run UI validation commands (`typecheck`, `test`, `build`) when feasible.
3. Deploy as normal frontend content update.

Rollback: revert the summary/highlights line changes in `src/features/journey/data.ts`.
